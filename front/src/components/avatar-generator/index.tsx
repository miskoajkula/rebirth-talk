...
type AvatarGeneratorProps = {
  onChange: (avatar: Avatar) => void; defaultAvatar: Avatar; renderPalletsInModal: boolean;
}

const AvatarGenerator = ({onChange, defaultAvatar, renderPalletsInModal = true}: AvatarGeneratorProps) => {
  ...
  const handleGenerate = (callOnChange = true) => {
    ...
    if (callOnChange) {
      onChange({
        colors: selectedPalette, name: randomName,
      })
    }
  };
  ...
  const onSaveButtonClick = () => {
    if(defaultAvatar?.name !== name ) {
      onChange({
        colors: selectedPalette, name: name,
      })
    }
  }
  ...
  const renderPallets = (isVertical = true) => {
    ...
    return <div className={style}>
      {avatarPallets.map((palette, index) => (<div
        key={index}
      ...
      >
        ...
        <div className="flex space-x-2">
          {palette.colors.map((color, idx) => (<div
            key={idx}
  ...
  return (<div className="flex overflow-scroll bg-transparent flex-col items-center justify-center p-4">
    {/* Avatar Preview */}
    <div className={"relative"}>
      ...
      <FaPalette onClick={() => setModal(true)}
                 className={"w-8 h-8 bg-[#00382e] text-white rounded-2xl p-2 absolute top-0 right-0 hover:cursor-pointer hover:scale-95"}/>
    </div>
